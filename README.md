<div align="center">
  <h1><img align="middle" alt="Deskflow ASM" height="72" src="docs/images/readme-icon.png" /> Deskflow Active Session</h1>
...
Release artifacts are signed with the configured Apple Development identity when the repository signing secrets are available. I currently do not have a paid personal Apple Developer account available for Developer ID distribution signing and notarization. The releases are therefore not notarized, so macOS displays a Gatekeeper warning when they are redistributed. CI falls back to an ad-hoc signature when no signing secrets are configured.
